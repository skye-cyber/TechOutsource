...
if (isset($_POST["postJob"])) {
    ...
    $dateline = test_input($_POST["deadline"]);


    // Check if the date is in the future
    if (strtotime($dateline) > strtotime(date("Y-m-d"))) {
        echo "The date is in the future.";
    } else {
        echo "Invalid date. Please enter a future date.";
    }



    $sql = "INSERT INTO job_offer (title, type, description, budget, skills, special_skill, deadline,e_username,valid)
    VALUES ('$title', '$type', '$description','$budget','$skills','$special_skill','$dateline','$username',1)";
